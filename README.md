...
同一把量具还读**证据指针**：行内代码里的路径、路径:行号、路径.ext.属性、测试文件.py::测试函数名、裸 test_ 函数名，都是一条可解析的断言（这几类拼法故意写在正文里而不是代码片段里，因为读者把代码片段当断言——写它们的那一段自己就会被自己校验），全仓 19 个文档在干净检出里共 782 条断言（在跑过构建与评测的开发机上会更多——这条量具目前仍把本机目录当仓库内容，是已登记的下一个缺陷），逐条真去解析——文件必须存在、行号不能超过文件行数、`::` 后的测试必须真的定义在那个文件里、属性名必须出现在被引文件里。豁免不是「这条不管了」，而是「这条的语义已经变了」，所以每条豁免必须带非空理由、必须仍被某个文档引用（否则算陈旧登记），目前共六类：不是仓库路径、构建产物、计划里承诺但未交付、被引用来说明它已过期、主动退役、计划里否掉的拼法。当一个名字已经改名而记录必须留住它时，把死名字写在正文里、把活指针放在行内代码里：读者只校验代码片段，正文只校验带斜杠的路径，于是历史留住了、悬空指针也没有。明确没管的：围栏代码块内部、代码片段内部、快照与范围外文档里的悬空指针；无扩展名的模块名和裸文件名不算断言（`127.0.0.0/8`、`tools/call`、`loop.py` 都会被误判）；`--check` 尚未接入 CI。
