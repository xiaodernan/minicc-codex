...
合并成功、子任务失败、合并器抛错、父任务被取消或崩溃、以及从 worker 进程镜像回来的重连收尾，都算；这份汇总幂等（记录带 `children_rolled_up`，重启后从任务索引恢复不会再加一次；汇总过的结果负载会**自己声明**已汇总，所以重连时既不会少记子树、也不会二次相加），所以同一个子树既不会少记也不会双计。另一条同向的口径：结果为空的
