...
文档里的交叉引用用 `python scripts/doc_pointers.py --check` 复现：把 `docs/*.md` 与 README 里每条 `见/参见` 指针和每个相对链接真去解析一次——批号/节号必须落在一个**存在的标题**上，指针里同时点名编号时（`见「第十八批 M8-T34」`）该编号必须在**那一节的标题或表格行标签**里被声明过，只在正文里出现过不算数（指针自己就写着那个编号）。中文里「见」是常见后缀（可见、意见、见证），所以只抽带定位符或编号的片段。节号与批号**两种写法都认**：`第四节` 与 `第8节` 都是定位符，标题侧 `## 四、` 与 `## 8.` 都能答它——只教其中一侧，读得懂「第8节」却没有标题能答它，两条真断言会立刻变成两条假红。指针还**必须把文件名写进句子里**：昵称不是可查路径，`见交付说明第8节` 该红，写成链接形式才可核对。**每个标记必须落进七箱之一并在总结行逐箱报数**（当前定稿读数：`checked 48 of 163`，七箱 `checked=48`、`citation-without-locator=16`、`carried-by-link-reader=9`、`carried-by-evidence-reader=2`、`code-span-names-a-file=6`、`path-in-prose=3`、`word-interior=79`），分箱之和还要与独立重数对账，不等报 `RECONCILE`——因为「其余计入一个整数」这种交代方式盖过一件事：`可见价值低于 M6–M8 任何一项` 曾被抽成指针并**绿着通过**（M6、M8 确实都在这个文件里），假指针比缺口糟，因为它往已核验那一栏记了一笔。定稿规则：span 里带节号/批号/附录号的一律算引用；只带编号的必须站在引用位置上（以引用词、`「`、`[`、编号本身开头）；span 为空时看未掩码的原文里紧跟的是代码片段还是链接——那两条分别归证据阅读器与链接阅读器，不算缺口。**托管要验一次发票**：那句「归证据阅读器」只在片段真的是对方会解析的形状时才成立；上一批这一箱记着 7 条，逐条问对方认不认，只有 2 条是真交接，剩下 5 条是裸文件名和被忽略目录下的路径——证据阅读器故意不判它们（否则正文里几十处 `loop.py` 简写会一夜变红），于是它们一直躺在「别人会查」那一栏。现在它们进 `code-span-names-a-file`，由指针阅读器自己答：带目录的按链接阅读器同一套存在性问，裸名允许命中任意一个被跟踪同名文件，被 git 忽略的算生成产物，还带行号范围的不得超过被引文件行数；既不像文件也不是证据形状的片段（只写着字段名的那种）落 `citation-without-locator`，不许落进词内箱——那一箱的含义是「根本不是引用」。剩下的 `citation-without-locator` 箱（`口径见下方注记`、`见上表末格`、`见本批末尾`）是**有名字、有条数、没有判据**的一格：16 条逐类量过之后确认「注记/表格某一格/相对位置/占位符 `第N批`」这几种落点都没有可核对的声明位置，任何「存在即通过」的判据都是自我满足，所以不假装它已解决。链接阅读器也先掩码：**把链接形状写进反引号当例子不是断言**（这条是记录自己撞出来的——撤掉掩码，语料里 5 个被引用的形状会全被当成断言）；代价与指针侧同一条口径：只在代码片段里出现的坏链接这里也看不见。
