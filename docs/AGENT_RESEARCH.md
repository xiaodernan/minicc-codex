...
- [Claude Code subagents](https://docs.anthropic.com/en/docs/claude-code/sub-agents)：子 agent 适合有明确职责的独立工作，结果需要回到主任务；本项目按复杂度自动触发有界 fan-out，并把只读证据交回父 Agent，避免无边界递归。
- [OpenAI Codex](https://developers.openai.com/codex/) 与 [Agents orchestration](https://developers.openai.com/api/docs/guides/agents/orchestration)：编排可以选择 handoff 或 agents-as-tools，取决于专家是否接管任务以及管理器是否保留最终回答；本项目采用后者的有界形态，由父 Agent 接收只读证据并保留实现、验证和最终交付控制权。
...
| [OpenAI Agents orchestration](https://developers.openai.com/api/docs/guides/agents/orchestration) | “handoff”适合让专家接管分支；“agents as tools”适合经理保留最终回答。只有职责、工具或策略真正变化时才拆分专家。 | 复杂任务自动路由到固定职责的只读侦察子任务，父 Agent 保留最终回答、写入和验证控制权；手动批量仍可作为高级入口。 |
...
用户任务
  ...
  -> complexity router: simple task OR bounded read-only fan-out
  -> child evidence merge -> parent Agent loop: context / plan / tool round / replan / verify
  -> 只读工具和侦察子任务并行，写入和命令顺序执行
...
- 复杂任务会自动进入固定 2-3 节点的只读 fan-out；没有无约束的自发 subagent 递归，写入始终由父 Agent 顺序执行。
