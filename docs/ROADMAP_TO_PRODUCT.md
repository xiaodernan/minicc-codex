...
| 标准 | 结论 | 证据 |
| --- | --- | --- |
| M1-1 `pytest -q` 全绿 | ✅（Windows 这条腿） | 最新基线 `.venv` 全量 **998 passed**（`-W error`，exit 0；同批上一基线 997、995、994、987、982、938、929、927、919、917、913）。wall time 这一列近五次同命令测量：**222.92s**（994 条）、**230.90s**（同一份 994 条复跑）、**339.84s**（995 条）、**245.65s**（997 条）、**306.94s**（998 条），更早还有 238.58s / 280.65s / 518.39s——**220-340s 之间摆动，而 339.84s 那次只比 222.92s 那次多 1 条测试**，所以时长口径本身不可信（本机同时有别的 agent 会话在跑，但逐次负载我没量过，**归因未证实**）。结论不变：**测试条数与 exit code 是口径，wall time 只用来记「这次跑了多久」**，单次时长不做性能结论。Ubuntu 那条腿本机不可用，只有 CI 能证，**不在此声明** |
