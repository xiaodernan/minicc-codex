...
| M8-T34 给前缀分派与流式路由补「会读流的探针」，把 `_UNPROBED_FAMILIES` 逐条清空（**下一步，可直接开工，不需要授权**） | ⏳ 待办 | M8-T33 把「100% 之外的洞」登记成了数据，但登记不是覆盖。要做的事：① 用 live harness 先提交一个任务拿到 `task_id`；② `/api/tasks/<id>`（GET 详情）与 `/api/tasks/<id>/resume`、`/cancel`（POST）走普通请求，沿用 `assert_answered`（`<500`，拒时带稳定 code）；③ `/api/tasks/<id>/events` 必须**流式读**：`urlopen` 后读若干字节、拿到至少一个 SSE 帧即关闭，断言「拿到帧」或「结构化拒绝」，绝不允许挂到 timeout；不存在的 `task_id` 要断 `task_not_found`；④ 每关掉一族就从 `_UNPROBED_FAMILIES` 删一项（那张表现在会红着提醒），并把 `_route_table` 的枚举扩到这些动态路由，让总数仍带下限与「具名路由在册」；⑤ 真无法安全探测的族（例如必须有真 worker）**登记原因**而不是从表里悄悄删掉。**环境约束照旧**：同时只跑一个 pytest 进程（共享 `~/.minicc/tasks.sqlite3` 与固定测试端口）、跑全量期间不改源码、提交前扫 staged diff 的密钥、三个 `inspect_*.py` 永不 stage、不用 `--no-verify`。当前基线 **1007 passed**（`-W error`，exit 0） |
