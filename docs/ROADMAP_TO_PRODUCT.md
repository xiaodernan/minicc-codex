...
**M8-T50 落地的第二版（收口之后）**：上一版的错误是让 faulthandler 的 exit=True 去「判红」，它只会把进程打死（Windows + pytest 下表现为 access violation，整场报告一起丢）。这一版把两件事分开：监视线程到点只打印栈（不退出、不改世界），红由 pytest_runtest_makereport 用 pytest 自己量出的 call 时长判定（超过 MINICC_TEST_HANG_LIMIT 即失败，消息里说明缘由）。新门 tests/test_hang_watchdog.py 三条，两半都要：预算内=不红也不打印（否则「什么都杀」的实现也能满足正向）；超预算=nested 会话保留报告并把慢的那条报成 failed；再加一条环境语义（没设=900s 默认；设了但空/非法/非正=关）。**装上看门狗之后全量冷跑：1088 passed / 401.54s / exit 0**（1085 + 3 条新门，默认上限全程没误伤任何一条门）。门数与全量数都取实测，不写推算。
