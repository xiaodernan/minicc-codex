...
| 标准 | 结论 | 证据 |
| --- | --- | --- |
| M1-1 `pytest -q` 全绿 | ✅（Windows 这条腿） | 最新基线 `.venv` 全量 **1001 passed**（`-W error`，exit 0；同批上一基线 999、998、997、995、994、987、982）。**破 1000 条不是成就本身**——这一千里有围栏（M8-T27）、有把规则边界逐名列成数据的门（M8-T29 的 11+10 名单），它们不新增产品能力，只是让「已承诺的行为」变得可以被机器复查；这条要记的是**口径**而不是规模。wall time 这一列近七次同命令测量：**222.92s**（994）、**230.90s**（994 复跑）、**339.84s**（995）、**245.65s**（997）、**306.94s**（998）、**226.26s**（999）、**411.10s**（1001），更早还有 238.58s / 280.65s / 518.39s——**220-411s 之间摆动，1001 次比 999 次多 2 条却慢了近 8 分钟**，所以时长口径本身不可信（本机同时有别的 agent 会话在跑，逐次负载我没量过，**归因未证实**）。结论不变：**测试条数与 exit code 是口径，wall time 只用来记「这次跑了多久」**。Ubuntu 那条腿本机不可用，只有 CI 能证，**不在此声明** |
