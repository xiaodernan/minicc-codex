...
**账要算清**：本批因此多了一条门，全量数从 1083 变到 **1084 是推算值**（+1 条新测试，未重跑全量——收口这台机器当时的负载让一次全量跑到 12 分钟没收尾）。缺测就说缺测，不补一个像量过的数。
