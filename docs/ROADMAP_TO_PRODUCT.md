...
#### M8-T46 候选（下一批）：仓库里还剩两处秒表，判别边际没量过

`tests/test_http_route_inventory.py` 里那条 `elapsed < 10` 与 `tests/test_core_tools.py` 里那条 `elapsed < 15`：本批没有找到它们的结构见证，所以**没有动**——没有实测过「多慢算回归」的秒表，改它和留着它都是猜。下一批的先量动作和这里一样：把每条的时间读数在空闲与负载两种条件下各取一组，看判别边际还剩多少，再决定搬家还是留活体线。
