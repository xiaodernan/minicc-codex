(() => {
  ...
  function refreshIcons(root) {
    return window.refreshIcons?.(root);
  }
  ...
  var GAME_DIFFICULTIES = {
    normal: { hpMultiplier: 0.9, initialSun: 200, speedMultiplier: 0.9, spawnDelayMultiplier: 1.16, waveBonus: -1 },
    hard: { hpMultiplier: 1, initialSun: 175, speedMultiplier: 1, spawnDelayMultiplier: 1, waveBonus: 0 },
    nightmare: { hpMultiplier: 1.58, initialSun: 110, speedMultiplier: 1.28, spawnDelayMultiplier: 0.56, waveBonus: 6 }
  };
  var savedGameDifficulty = Object.prototype.hasOwnProperty.call(GAME_DIFFICULTIES, localStorage.getItem("minicc-game-difficulty")) ? localStorage.getItem("minicc-game-difficulty") : "hard";
  var WAVE_TARGET = (wave, difficulty = savedGameDifficulty) => 7 + wave * 2 + GAME_DIFFICULTIES[difficulty].waveBonus + (difficulty === "nightmare" ? wave + Math.floor((wave + 1) / 2) : 0);
  var game = { running: false, paused: false, pauseReasons: /* @__PURE__ */ new Set(), frame: 0, score: 0, sun: GAME_DIFFICULTIES[savedGameDifficulty].initialSun, wave: 1, waveTarget: WAVE_TARGET(1), waveSpawned: 0, totalSpawned: 0, waveClearTimer: 0, elapsed: 0, selected: null, hoverCell: null, shovel: false, seedCooldowns: {}, skillCooldowns: {}, energy: 60, rallyTimer: 0, timeStopTimer: 0, plants: [], zombies: [], defeated: [], suns: [], shots: [], particles: [], impacts: [], popups: [], mowers: [], combo: 0, comboTimer: 0, bestCombo: 0, bannerTimer: 0, bannerText: "", bannerColor: "#ffe27c", dangerPulse: 0, last: 0, spawnTimer: 0, skyTimer: 0, dangerTimer: 0, difficulty: savedGameDifficulty, autoSun: localStorage.getItem("minicc-game-auto-sun") !== "off", musicOn: localStorage.getItem("minicc-game-sound") !== "off", volume: Math.max(0, Math.min(100, Number(localStorage.getItem("minicc-game-volume")) || 70)), audio: null, hudAt: 0, flagRows: new Uint8Array(5), renderStats: { frames: 0, fps: 0, lastFrameMs: 0, maxFrameMs: 0, longFrames: 0, frameSamples: [], recentSamples: new Array(60), recentSampleIndex: 0, recentSampleCount: 0, recentLongFrames: 0, windowStartedAt: 0, windowFrames: 0, indexRebuilds: 0, rowQueries: 0, rowCandidates: 0, drawCalls: 0, plantDraws: 0, zombieDraws: 0, animationSwitches: 0 } };
  var ZOMBIE_BODY_COLORS = { walker: "#526b5e", roadblock: "#53677d", bucket: "#566273", runner: "#9b5d4f", imp: "#b45d4e", football: "#334b68", miner: "#72574a", polevault: "#57785d", flag: "#754d6c", dancer: "#8d3f68", newspaper: "#806c50", conehead: "#b76b4d", witch: "#563d70", dragon: "#8a453f", gargantuar: "#694450", backup: "#a15c72" };
  var gameLayout = { left: 78, top: 72, cellW: 70, cellH: 65, rows: 5, cols: 9 };
  var GAME_LOGICAL_WIDTH = 720;
  var GAME_LOGICAL_HEIGHT = 420;
  var GAME_MAX_PARTICLES = 180;
  var GAME_MAX_POPUPS = 48;
  var GAME_MOWER_TRIGGER_X = 92;
  var GAME_MOWER_SPEED = 0.62;
  var GAME_MOWER_CLEAR_RADIUS = 36;
  var GAME_MOWER_EXIT_X = GAME_LOGICAL_WIDTH + 46;
  var GAME_COMBO_WINDOW = 1800;
  var GAME_SKILLS = {
    pulse: { cost: 25, cooldown: 7e3, label: "game.skillPulse", hint: "game.skillPulseHint" },
    sun: { cost: 35, cooldown: 1e4, label: "game.skillSun", hint: "game.skillSunHint" },
    rally: { cost: 45, cooldown: 14e3, label: "game.skillRally", hint: "game.skillRallyHint" },
    timeStop: { cost: 55, cooldown: 18e3, label: "game.skillTimeStop", hint: "game.skillTimeStopHint" }
  };
  var gameRows = {
  ...
  function cacheGameEntityPosition(entity) {
    ...
    if (position) {
      ...
      if (entity.type && entity.x === void 0) entity.x = position.x;
      if (entity.type && entity.y === void 0) entity.y = position.y;
    }
    return entity;
  }
  ...
  function anyIndexedEntity(kind, predicate) {
    return Boolean(firstIndexedEntity(kind, predicate));
  }
  ...
  function anyRowEntity(kind, row, predicate) {
    return Boolean(firstRowEntity(kind, row, predicate));
  }
  ...
  var gameCellPositions = Array.from(
    { length: gameLayout.rows },
    (_, row) => Array.from({ length: gameLayout.cols }, (_2, col) => ({
      x: gameLayout.left + col * gameLayout.cellW + 35,
      y: gameLayout.top + row * gameLayout.cellH + 31
    }))
  );
  var gameRender = { canvas: null, ctx: null, background: null, dpr: 1, effects: "high", deviceProfile: null };
  ...
  function buildGameBackground() {
    ...
    sky.addColorStop(0, "#9bd9df");
    sky.addColorStop(0.35, "#d7e7b2");
    sky.addColorStop(0.36, "#659b58");
    sky.addColorStop(1, "#315744");
    ctx.fillStyle = sky;
    ctx.fillRect(0, 0, GAME_LOGICAL_WIDTH, GAME_LOGICAL_HEIGHT);
    ctx.fillStyle = "rgba(255,255,255,.18)";
    ctx.beginPath();
    ctx.arc(605, 42, 29, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#253b45";
    ctx.fillRect(0, 0, GAME_LOGICAL_WIDTH, 58);
    ctx.fillStyle = "#eef3cf";
    ctx.font = "700 12px Manrope, sans-serif";
    ctx.fillText("BACKYARD", 18, 23);
    ctx.fillStyle = "#d9b16e";
    ctx.fillRect(48, 28, 17, 23);
    ctx.fillStyle = "#693f38";
    ctx.beginPath();
    ctx.moveTo(44, 29);
    ctx.lineTo(57, 16);
    ctx.lineTo(70, 29);
    ...
    ctx.fillRect(53, 39, 7, 12);
    for (let row = 0; row < gameLayout.rows; row += 1) for (let col = 0; col < gameLayout.cols; col += 1) {
      const x = gameLayout.left + col * gameLayout.cellW, y = gameLayout.top + row * gameLayout.cellH;
      ctx.fillStyle = (row + col) % 2 ? "#75b866" : "#83c573";
      roundedRect(ctx, x + 2, y + 2, 66, 61, 8);
      ctx.fill();
      ctx.strokeStyle = "rgba(221, 246, 151, .18)";
      ctx.stroke();
    }
    const laneGlow = ctx.createLinearGradient(78, 72, 78, 397);
    laneGlow.addColorStop(0, "rgba(255,255,255,.06)");
    laneGlow.addColorStop(0.5, "rgba(255,255,255,0)");
    laneGlow.addColorStop(1, "rgba(12,38,29,.14)");
    ctx.fillStyle = laneGlow;
    ctx.fillRect(78, 72, 630, 325);
    ctx.strokeStyle = "rgba(232, 250, 176, .14)";
    ctx.lineWidth = 1;
    for (let row = 0; row <= gameLayout.rows; row += 1) {
      const y = gameLayout.top + row * gameLayout.cellH;
      ctx.beginPath();
      ctx.moveTo(gameLayout.left, y);
      ctx.lineTo(708, y);
      ctx.stroke();
    }
    ctx.fillStyle = "rgba(240, 213, 140, .28)";
    ctx.fillRect(674, 60, 3, 360);
    ctx.fillStyle = "rgba(255,255,255,.07)";
    ctx.fillRect(678, 60, 1, 360);
  ...
  function recordGameFrame(now, startedAt) {
    ...
    if (now - stats.windowStartedAt >= 1e3) {
      stats.fps = stats.windowFrames * 1e3 / (now - stats.windowStartedAt);
  ...
  var plantCost = { peashooter: 100, sunflower: 50, wallnut: 50, repeater: 180, cherrybomb: 150, icepeashooter: 175, firepeashooter: 175, twinpea: 225, kernelpult: 100, pumpkin: 125, spikeweed: 100, gloomshroom: 150, potatomine: 25, threepeater: 325, jalapeno: 125, magnetshroom: 100, garlic: 50, squash: 50, gatlingpea: 350 };
  var PLANT_TYPES = Object.keys(plantCost);
  var plantHealth = { peashooter: 7, sunflower: 6, wallnut: 24, repeater: 8, cherrybomb: 4, icepeashooter: 7, firepeashooter: 7, twinpea: 10, kernelpult: 8, pumpkin: 32, spikeweed: 10, gloomshroom: 9, potatomine: 3, threepeater: 8, jalapeno: 4, magnetshroom: 7, garlic: 8, squash: 6, gatlingpea: 9 };
  var plantColor = { peashooter: "#62b5a0", sunflower: "#f6c453", wallnut: "#ad7556", repeater: "#75c77b", cherrybomb: "#dd6d73", icepeashooter: "#8bc9e8", firepeashooter: "#f07855", twinpea: "#8bd15f", kernelpult: "#e8bf65", pumpkin: "#e29b45", spikeweed: "#8dbf62", gloomshroom: "#8563aa", potatomine: "#a9bd72", threepeater: "#72c789", jalapeno: "#ef765f", magnetshroom: "#b187d5", garlic: "#f3e1b4", squash: "#e2a848", gatlingpea: "#4db878" };
  var plantCooldown = { peashooter: 250, sunflower: 900, wallnut: 700, repeater: 450, cherrybomb: 900, icepeashooter: 850, firepeashooter: 850, twinpea: 950, kernelpult: 700, pumpkin: 1050, spikeweed: 650, gloomshroom: 1100, potatomine: 650, threepeater: 1100, jalapeno: 1200, magnetshroom: 900, garlic: 800, squash: 800, gatlingpea: 1400 };
  var plantProfiles = {
    ...
    kernelpult: { interval: 1500, shots: 1, damage: 1, slow: 0, butterChance: 0.28, pierce: 1 },
  ...
  function plantHasSunlight(plant) {
    ...
    return game.plants.some((container) => [container, container.underPlant].filter(Boolean).some((candidate) => candidate !== plant && candidate.type === "sunflower" && Math.abs(candidate.row - plant.row) <= 1 && Math.abs(candidate.col - plant.col) <= 1));
  }
  function plantFireInterval(plant, profile) {
    const sunlightMultiplier = plant.sunlightBoost ? 0.78 : 1;
    const rallyMultiplier = game.rallyTimer > 0 ? 0.55 : 1;
    return profile.interval * sunlightMultiplier * rallyMultiplier;
  }
  var zombieProfiles = {
    walker: { hp: 5, speed: 0.02, growth: 1e-3, attackInterval: 1e3, score: 1 },
    backup: { hp: 4, speed: 0.025, growth: 8e-4, attackInterval: 900, score: 1 },
    roadblock: { hp: 12, speed: 0.013, growth: 65e-5, attackInterval: 670, score: 3, armor: 5, barricade: true },
    conehead: { hp: 9, speed: 0.021, growth: 9e-4, attackInterval: 900, score: 2, armor: 3, cone: true },
    imp: { hp: 3, speed: 0.044, growth: 11e-4, attackInterval: 1250, score: 2, dash: true, leap: true },
    scout: { hp: 7, speed: 0.03, growth: 9e-4, attackInterval: 820, score: 4, dash: true, mark: true },
    storm: { hp: 11, speed: 0.016, growth: 6e-4, attackInterval: 740, score: 7, storm: true },
    runner: { hp: 4, speed: 0.036, growth: 8e-4, attackInterval: 1150, score: 2, dash: true },
    polevault: { hp: 10, speed: 0.025, growth: 7e-4, attackInterval: 700, score: 5, vault: true },
    bucket: { hp: 21, speed: 0.011, growth: 45e-5, attackInterval: 620, score: 5, armor: 8, bucket: true },
    football: { hp: 18, speed: 0.024, growth: 55e-5, attackInterval: 430, score: 6, armor: 5, charge: true },
    miner: { hp: 9, speed: 0.018, growth: 7e-4, attackInterval: 850, score: 5, burrow: true },
    flag: { hp: 6, speed: 0.027, growth: 1e-3, attackInterval: 900, score: 3, banner: true },
    dancer: { hp: 13, speed: 0.019, growth: 55e-5, attackInterval: 650, score: 7, summon: true },
    newspaper: { hp: 8, speed: 0.017, growth: 8e-4, attackInterval: 760, score: 4, armor: 3, enrage: true },
    gargantuar: { hp: 48, speed: 8e-3, growth: 35e-5, attackInterval: 360, score: 12, armor: 8, giant: true, smash: true },
    witch: { hp: 16, speed: 0.014, growth: 5e-4, attackInterval: 800, score: 9, curse: true },
    dragon: { hp: 26, speed: 0.01, growth: 35e-5, attackInterval: 560, score: 10, armor: 2, breath: true },
    shield: { hp: 14, speed: 0.016, growth: 6e-4, attackInterval: 720, score: 6, armor: 12, guard: true }
  };
  function gameDifficulty() {
    return GAME_DIFFICULTIES[game.difficulty] || GAME_DIFFICULTIES.hard;
  }
  ...
  function toggleGamePause() {
    setGamePauseReason("manual", !game.pauseReasons.has("manual"));
  }
  ...
  function formatGameTime(value) {
    const total = Math.max(0, Math.floor(value / 1e3));
  ...
  function updateGameHud(force = true) {
    ...
    $$(".game-skill").forEach((button) => {
      ...
      button.style.setProperty("--skill-cooldown", `${Math.ceil(remaining / 1e3)}s`);
      button.title = cooling ? `${t(skill.label)} \xB7 ${Math.ceil(remaining / 1e3)}s` : `${t(skill.label)} \xB7 ${energy}/${skill.cost}`;
    });
    $$(".seed-card").forEach((card) => {
      ...
      card.style.setProperty("--seed-cooldown", `${Math.ceil(remaining / 1e3)}s`);
    });
  }
  function activateGameSkill(type) {
    ...
    if (remaining > 0) {
      setGameStatus("game.skillCooldown");
      return false;
    }
    if ((game.energy || 0) < skill.cost) {
      setGameStatus("game.skillNeedEnergy");
      return false;
    }
    ...
    if (type === "pulse") {
      ...
      addGameParticle(390, 230, "#d7c7ff", 56, 0.38);
  ...
  function cellPosition(row, col) {
    return gameCellPositions[row]?.[col] || { x: gameLayout.left + col * gameLayout.cellW + 35, y: gameLayout.top + row * gameLayout.cellH + 31 };
  }
  ...
  function addGameParticle(x, y, color, count = 6, speed = 0.08) {
    for (let i = 0; i < count; i += 1) game.particles.push({ x, y, vx: (Math.random() - 0.5) * speed, vy: (Math.random() - 0.7) * speed, life: 420 + Math.random() * 360, maxLife: 780, size: 2 + Math.random() * 3, color });
  ...
  function addGamePopup(x, y, text, color = "#fff1b0", life = 850) {
    game.popups.push({ x, y, text, color, life, maxLife: life, vy: -0.025 });
  ...
  function gameVolume() {
    return game.musicOn ? 0.1 * (game.volume / 100) : 1e-3;
  }
  function playGameSound(kind) {
    ...
    try {
      if (!game.audio) {
        ...
        master2.gain.value = gameVolume();
        master2.connect(ctx2.destination);
        game.audio = { ctx: ctx2, master: master2, musicTimer: null, step: 0, lastSound: {} };
      }
      const { ctx, master } = game.audio;
      if (ctx.state === "suspended") ctx.resume();
      const sounds = {
        collect: { notes: [660, 880], type: "sine", duration: 0.16, step: 0.055, volume: 0.14 },
        plant: { notes: [330, 494], type: "triangle", duration: 0.2, step: 0.07, volume: 0.14 },
        shoot: { notes: [180, 230], type: "square", duration: 0.1, step: 0.035, volume: 0.08 },
        hit: { notes: [145], type: "square", duration: 0.08, step: 0, volume: 0.08 },
        explode: { notes: [130, 92, 58], type: "sawtooth", duration: 0.34, step: 0.07, volume: 0.2 },
        wave: { notes: [392, 523, 659], type: "triangle", duration: 0.42, step: 0.1, volume: 0.16 },
        victory: { notes: [523, 659, 784, 1046], type: "triangle", duration: 0.7, step: 0.11, volume: 0.18 },
        gameover: { notes: [220, 165, 110], type: "sawtooth", duration: 0.55, step: 0.13, volume: 0.16 },
        danger: { notes: [110, 98], type: "square", duration: 0.18, step: 0.08, volume: 0.1 },
        mower: { notes: [196, 294, 392], type: "sawtooth", duration: 0.36, step: 0.07, volume: 0.13 }
      };
      const sound = sounds[kind] || sounds.hit;
      const now = ctx.currentTime;
      const minInterval = { hit: 0.055, shoot: 0.08, collect: 0.04 }[kind] || 0;
      ...
      sound.notes.forEach((frequency, index) => {
        ...
        gain.gain.linearRampToValueAtTime(sound.volume, start + 0.012);
        gain.gain.exponentialRampToValueAtTime(1e-3, start + sound.duration);
  ...
  function startGameMusic() {
    ...
    game.audio.musicTimer = window.setInterval(() => {
      ...
      oscillator.frequency.setValueAtTime(melody[game.audio.step++ % melody.length], start);
      gain.gain.setValueAtTime(1e-3, start);
      gain.gain.linearRampToValueAtTime(0.05, start + 0.02);
      gain.gain.exponentialRampToValueAtTime(1e-3, start + 0.3);
      oscillator.connect(gain);
      gain.connect(master);
      oscillator.start(start);
  ...
  function stopGameMusic() {
    if (game.audio?.musicTimer) {
      clearInterval(game.audio.musicTimer);
      game.audio.musicTimer = null;
    }
  }
  function updateSoundButton() {
    const button = $("#gameSoundToggle");
    const volume = $("#gameVolume");
    if (button) {
      button.textContent = t(game.musicOn ? "game.soundOn" : "game.soundOff");
      button.classList.toggle("muted", !game.musicOn);
      button.setAttribute("aria-pressed", String(game.musicOn));
    }
    if (volume) volume.value = String(game.volume);
  }
  function setGameVolume(value) {
    game.volume = Math.max(0, Math.min(100, Number(value) || 0));
    localStorage.setItem("minicc-game-volume", String(game.volume));
    if (game.audio?.master) game.audio.master.gain.setTargetAtTime(gameVolume(), game.audio.ctx.currentTime, 0.02);
  }
  function toggleGameSound() {
    game.musicOn = !game.musicOn;
    localStorage.setItem("minicc-game-sound", game.musicOn ? "on" : "off");
    if (game.audio?.master) game.audio.master.gain.setTargetAtTime(gameVolume(), game.audio.ctx.currentTime, 0.02);
    if (game.musicOn && game.volume) {
      playGameSound("collect");
      startGameMusic();
    } else stopGameMusic();
    updateSoundButton();
  }
  function setGameStatus(key) {
    $("#gameStatus").textContent = t(key);
  }
  function setGamePaused(paused) {
    setGamePauseReason("visibility", paused);
  }
  ...
  function initGame() {
    ...
    game.impacts = [];
    game.popups = [];
    game.mowers = Array.from({ length: gameLayout.rows }, (_, row) => ({ row, x: 57, active: false, used: false, seed: Math.random() * 1e3 }));
    game.combo = 0;
    game.comboTimer = 0;
    game.bestCombo = 0;
    game.bannerTimer = 0;
  ...
  function drawSun(ctx, sun) {
    const pulse = 1 + Math.sin(sun.age / 230) * 0.08;
    const glow = gameRender.effects === "low" ? 0 : 18;
    ctx.save();
    ctx.translate(sun.x, sun.y);
    ctx.scale(pulse, pulse);
    ctx.shadowColor = "rgba(255, 215, 84, .75)";
    ctx.shadowBlur = glow;
  ...
  function drawMower(ctx, mower, now) {
    ...
    const vibration = mower.active ? Math.sin((now + mower.seed) / 34) * 1.8 : Math.sin((now + mower.seed) / 480) * 0.6;
    ctx.save();
  ...
  function drawPlacementPreview(ctx, now) {
    ...
    const valid = game.shovel ? Boolean(existing) : Boolean(game.selected && (!existing || covering) && game.sun >= (plantCost[game.selected] || Infinity) && !(game.seedCooldowns[game.selected] > 0));
  ...
  function drawPlant(ctx, plant, now) {
    ...
    const breathe = 1 + Math.sin((now + plant.seed) / 620) * 0.025;
    const bob = Math.sin((now + plant.seed) / 430) * (plant.type === "spikeweed" ? 0.7 : 1.8);
    const leaf = (lx, ly, angle, color = "#3d9b5c") => {
      ctx.save();
      ctx.translate(lx, ly);
      ctx.rotate(angle);
      ctx.fillStyle = color;
    ...
    const stem = (height = 24, color = "#2f744d") => {
      ctx.strokeStyle = color;
      ctx.lineWidth = 5;
      ctx.lineCap = "round";
      ctx.beginPath();
      ctx.moveTo(0, 21);
      ctx.lineTo(0, 21 - height);
      ctx.stroke();
    };
    const peaFace = (color, mouth = 10) => {
      ctx.fillStyle = color;
      ctx.beginPath();
      ctx.arc(0, -17, 15, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = "#193c32";
      ctx.beginPath();
      ctx.arc(10, -17, mouth, -0.45, 0.45);
      ctx.fill();
      ctx.fillStyle = "#f3f4ca";
      ctx.beginPath();
    ...
    if (plant.sunlightBoost && gameRender.effects !== "low") {
      const pulse = 1 + Math.sin((now + plant.seed) / 180) * 0.08;
      ctx.save();
    ...
    ctx.fillStyle = "rgba(17, 51, 32, .3)";
    ctx.beginPath();
    ctx.ellipse(0, 25, 24, 7, 0, 0, Math.PI * 2);
    ctx.fill();
    switch (plant.type) {
      case "sunflower":
        stem();
        ...
        for (let i = 0; i < 10; i += 1) {
          const a = i * Math.PI / 5;
          ctx.fillStyle = i % 2 ? "#f4b83f" : "#ffd966";
          ctx.beginPath();
          ctx.ellipse(Math.cos(a) * 15, -8 + Math.sin(a) * 15, 7, 13, a, 0, Math.PI * 2);
          ctx.fill();
        }
      ...
      case "wallnut":
        ...
        ctx.lineWidth = 2;
        ctx.stroke();
        ctx.fillStyle = "#1d302c";
        ctx.beginPath();
        ctx.arc(-7, -11, 2, 0, Math.PI * 2);
        ctx.arc(7, -11, 2, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = "#1d302c";
        ctx.beginPath();
        ctx.arc(0, -2, 8, 0.15, Math.PI - 0.15);
        ctx.stroke();
        break;
      case "pumpkin":
        ctx.fillStyle = "#e27d31";
        ctx.beginPath();
        ctx.ellipse(0, -4, 24, 27, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = "#a94f29";
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.ellipse(-9, -4, 9, 25, 0, 0, Math.PI * 2);
        ctx.ellipse(9, -4, 9, 25, 0, 0, Math.PI * 2);
        ctx.stroke();
        ...
        break;
      case "cherrybomb":
        ctx.strokeStyle = "#4a744a";
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.moveTo(0, -20);
        ctx.quadraticCurveTo(4, -34, 14, -36);
        ctx.stroke();
        ctx.fillStyle = "#c94556";
        ctx.beginPath();
        ctx.arc(-10, -7, 14, 0, Math.PI * 2);
        ctx.arc(10, -7, 14, 0, Math.PI * 2);
        ctx.fill();
      ...
      case "potatomine":
        ...
        ctx.beginPath();
        ctx.ellipse(0, 5, 23, 16, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#db5a4d";
        ctx.beginPath();
      ...
      case "spikeweed":
        ...
        ctx.closePath();
        ctx.fill();
        ctx.fillStyle = "#d8eb9c";
        for (let i = -18; i <= 18; i += 9) {
          ctx.beginPath();
          ctx.arc(i, 13, 2, 0, Math.PI * 2);
          ctx.fill();
        }
        break;
      case "gloomshroom":
        stem(23, "#624478");
        ctx.fillStyle = "#7750a0";
        ctx.beginPath();
        ctx.arc(0, -17, 21, Math.PI, Math.PI * 2);
        ctx.lineTo(17, -9);
        ctx.quadraticCurveTo(0, -1, -17, -9);
        ctx.closePath();
        ctx.fill();
        ctx.fillStyle = "#d4a9ef";
        ctx.beginPath();
        ...
        break;
      case "jalapeno":
        ctx.fillStyle = "#ef654d";
        ctx.beginPath();
        ctx.moveTo(-4, 20);
        ctx.bezierCurveTo(-23, 5, -19, -22, 3, -28);
        ctx.bezierCurveTo(24, -23, 22, 9, 5, 20);
        ctx.closePath();
        ctx.fill();
        ctx.fillStyle = "#3c824d";
        ctx.fillRect(-4, -31, 9, 7);
        ...
        break;
      case "garlic":
        ctx.fillStyle = "#f1e5bc";
        ctx.beginPath();
        ctx.moveTo(0, -30);
        ctx.bezierCurveTo(-22, -23, -20, 11, 0, 20);
        ctx.bezierCurveTo(20, 11, 22, -23, 0, -30);
        ctx.fill();
        ctx.strokeStyle = "#c7b886";
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(0, -27);
        ctx.lineTo(0, 15);
        ctx.moveTo(-2, -22);
        ctx.quadraticCurveTo(-11, -4, -7, 10);
        ctx.moveTo(2, -22);
        ctx.quadraticCurveTo(11, -4, 7, 10);
        ctx.stroke();
        break;
      case "squash":
        ctx.fillStyle = "#e5ad43";
        ctx.beginPath();
        ctx.ellipse(0, -1, 25, 17, -0.1, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = "#9a6434";
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.ellipse(-10, -1, 9, 16, 0, 0, Math.PI * 2);
        ctx.ellipse(10, -1, 9, 16, 0, 0, Math.PI * 2);
        ctx.stroke();
        ctx.fillStyle = "#25382e";
        ctx.beginPath();
      ...
      case "kernelpult":
        ...
        leaf(-13, 13, -0.5);
        leaf(13, 14, 0.5);
        ctx.fillStyle = "#e8c54f";
        ctx.beginPath();
        ctx.ellipse(0, -18, 15, 18, -0.2, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#6aa84f";
        ctx.fillRect(-7, -35, 13, 5);
        break;
      case "magnetshroom":
        stem(21, "#704c80");
        ctx.fillStyle = "#bd79b9";
        ctx.beginPath();
        ctx.arc(0, -14, 20, Math.PI, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#303a4d";
        ctx.fillRect(-10, -9, 20, 5);
        break;
      case "icepeashooter":
        stem();
        leaf(-12, 13, -0.45, "#5da6ba");
        leaf(12, 14, 0.45, "#5da6ba");
        peaFace("#87d8e9");
      ...
      case "twinpea":
        ...
        ctx.fillStyle = "#193c32";
        ctx.beginPath();
        ctx.arc(-18, -16, 7, -0.4, 0.4);
        ctx.arc(18, -16, 7, Math.PI - 0.4, Math.PI + 0.4);
        ctx.fill();
        break;
      case "repeater":
      case "threepeater":
      ...
      case "peashooter":
        ...
        leaf(-12, 13, -0.45);
        leaf(12, 14, 0.45);
        peaFace(plant.type === "gatlingpea" ? "#45a86b" : plant.type === "threepeater" ? "#77c979" : plant.type === "repeater" ? "#70c77b" : "#61b59d", plant.type === "gatlingpea" ? 13 : 10);
        if (plant.type === "threepeater") {
          ctx.fillStyle = "#74c979";
          ctx.beginPath();
        ...
        if (plant.type === "gatlingpea") {
          ...
          ctx.fillRect(5, -28, 25, 7);
          ctx.fillRect(5, -18, 27, 7);
          ctx.fillRect(5, -8, 23, 7);
        }
        break;
      default:
        stem();
        leaf(-12, 13, -0.45);
        leaf(12, 14, 0.45);
        peaFace(plantColor[plant.type] || "#62b5a0");
    }
    if (plant.hp < plantHealth[plant.type]) {
      ctx.fillStyle = "rgba(18, 28, 24, .8)";
      ctx.fillRect(-20, 30, 40, 4);
      ctx.fillStyle = plant.hp / plantHealth[plant.type] > 0.4 ? "#78d69b" : "#f6a45e";
      ctx.fillRect(-20, 30, 40 * Math.max(0, plant.hp / plantHealth[plant.type]), 4);
    }
    ctx.restore();
  }
  function drawZombie(ctx, zombie, now) {
    ...
    const attackWindup = zombie.attacking && attackProgress > 0.52 ? Math.sin(Math.min(1, (attackProgress - 0.52) / 0.48) * Math.PI) : 0;
    ...
    const scale = giant ? 1.32 : zombie.type === "imp" ? 0.78 : 1;
    const body = ZOMBIE_BODY_COLORS[zombie.type] || ZOMBIE_BODY_COLORS.walker;
    const lean = attackWindup * -0.13 + dashPose * 0.08 + chargePose * 0.12 + hitStagger * 0.1;
    ...
    ctx.rotate(lean);
    ctx.scale(scale * (1 + chargePose * 0.04), scale * (1 - chargePose * 0.025));
    if (burrowed) ctx.globalAlpha = 0.62;
    ctx.fillStyle = "rgba(20, 27, 29, .32)";
    ctx.beginPath();
    ctx.ellipse(0, 27 + bob + leapPose * 13, 24 + Math.abs(stride) * 0.25, 7, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(-7, 14);
    ctx.lineTo(-12 + legSwing, 30 - legLift);
    ctx.moveTo(7, 14);
    ctx.lineTo(12 - legSwing, 30 - legLift * 0.55);
    ctx.stroke();
    const reach = attackWindup * 15 + chargePose * 7;
    const poseArmSwing = armSwing - attackWindup * 12 + dashPose * 5;
    ctx.strokeStyle = body;
    ctx.lineWidth = giant ? 6 : 4;
    ctx.beginPath();
    ctx.moveTo(-13, 3);
    ctx.lineTo(-23 - poseArmSwing - reach, 15 - attackWindup * 5);
    ctx.moveTo(13, 3);
    ctx.lineTo(23 + poseArmSwing + reach, 15 - attackWindup * 5);
    ctx.stroke();
    if (dashPose && gameRender.effects !== "low") {
      ctx.globalAlpha = 0.22;
      ctx.strokeStyle = zombie.type === "scout" ? "#f5cf63" : "#e57b70";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(-10, 10);
      ctx.lineTo(-42, 16);
    ...
    if (zombie.elite && gameRender.effects !== "low") {
      const elitePulse = 1 + Math.sin((now + zombie.seed) / 160) * 0.08;
      ctx.globalAlpha = 0.2 + Math.abs(Math.sin((now + zombie.seed) / 260)) * 0.18;
      ctx.strokeStyle = "#ffcf6b";
      ctx.lineWidth = 2;
      ctx.shadowColor = "#ff9b5f";
      ctx.shadowBlur = 12;
      ctx.beginPath();
      ctx.arc(0, -12, 31 * elitePulse, 0, Math.PI * 2);
      ctx.stroke();
      ctx.shadowBlur = 0;
      ctx.globalAlpha = 1;
    }
    if (zombie.attackFlashTimer > 0) {
      ctx.globalAlpha = Math.min(1, zombie.attackFlashTimer / 90);
      ctx.strokeStyle = "#ffd47a";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.arc(19 + reach, 12 - attackWindup * 5, 8 + attackWindup * 8, -0.7, 0.7);
      ctx.stroke();
      ctx.globalAlpha = 1;
    }
    if (skillPulse) {
      ctx.strokeStyle = zombie.type === "dragon" ? "rgba(255,145,84,.72)" : "rgba(195,168,255,.62)";
      ctx.lineWidth = 2;
      ctx.globalAlpha = 0.45 + Math.abs(Math.sin(now / 90)) * 0.4;
      ctx.beginPath();
      ctx.arc(0, -8, 28 + Math.abs(gait) * 4, 0, Math.PI * 2);
      ctx.stroke();
      ctx.globalAlpha = 1;
    }
    ctx.fillStyle = body;
    roundedRect(ctx, -16, -1, giant ? 34 : 31, 27, 8);
    ctx.fill();
    ctx.fillStyle = zombie.flashTimer > 0 ? "#fff7d7" : "#b9c7a9";
    ctx.beginPath();
    ctx.arc(0, -17, giant ? 18 : 15, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (zombie.type === "roadblock") {
      ...
      ctx.fillRect(-20, -33, 40, 7);
      ctx.fillStyle = "#b95942";
      ctx.fillRect(-15, -38, 30, 5);
    }
    if (zombie.type === "bucket") {
      ctx.fillStyle = "#aab4bd";
      ctx.fillRect(-18, -34, 36, 16);
      ctx.fillStyle = "#65717d";
      ctx.fillRect(-21, -20, 42, 4);
    }
    if (zombie.type === "conehead") {
      ctx.fillStyle = "#eb873e";
      ctx.beginPath();
      ctx.moveTo(0, -48);
      ctx.lineTo(-15, -27);
      ctx.lineTo(15, -27);
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = "#f4c15f";
      ctx.fillRect(-17, -29, 34, 5);
    }
    if (zombie.type === "football") {
      ctx.fillStyle = "#c76c50";
      ctx.beginPath();
      ctx.ellipse(0, -33, 21, 8, 0, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = "#dbe4ed";
      ctx.fillRect(-14, -35, 28, 3);
    }
    if (zombie.type === "miner") {
      ctx.fillStyle = "#d59c3d";
      ctx.beginPath();
      ctx.arc(0, -32, 18, Math.PI, Math.PI * 2);
      ctx.fill();
    ...
    if (zombie.type === "flag") {
      ctx.strokeStyle = "#e0b26e";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(17, 16);
      ctx.lineTo(17, -40);
      ctx.stroke();
      ctx.fillStyle = "#ef786c";
      ctx.beginPath();
      ctx.moveTo(18, -39);
      ctx.lineTo(36, -32);
      ctx.lineTo(18, -25);
    ...
    if (zombie.type === "witch") {
      ...
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = "#dcb5ff";
      ctx.beginPath();
      ctx.arc(0, -32, 5, 0, Math.PI * 2);
      ctx.fill();
    }
    if (zombie.type === "dragon") {
      ctx.fillStyle = "#d49b50";
      ctx.beginPath();
      ctx.moveTo(-18, -2);
      ctx.lineTo(-34, -18);
      ctx.lineTo(-25, 6);
    ...
    if (zombie.type === "gargantuar") {
      ctx.fillStyle = "#b8c4d1";
      ctx.fillRect(17, -2, 8, 31);
      ctx.fillStyle = "#d99a5e";
      ctx.beginPath();
      ctx.arc(21, 31, 9, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    if (zombie.armor > 0) {
      ctx.strokeStyle = "#e4c36b";
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.arc(0, -17, 20, Math.PI, Math.PI * 2);
      ctx.stroke();
    }
    if (zombie.hp < zombie.maxHp) {
      ctx.fillStyle = "rgba(25, 33, 26, .8)";
      ctx.fillRect(-21, 35, 42, 4);
      ctx.fillStyle = zombie.armor > 0 ? "#e9c66a" : "#e48374";
      ctx.fillRect(-21, 35, 42 * Math.max(0, zombie.hp / zombie.maxHp), 4);
    }
    ctx.restore();
  }
  function drawShot(ctx, shot, now) {
    ...
    if (shot.kind === "kernel") ctx.fillRect(-7, -5, 13, 10);
      ...
      ctx.beginPath();
      ctx.arc(0, 0, shot.kind === "fire" ? 7 : 6, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();
  }
  function drawImpact(ctx, impact) {
    ...
    const radius = impact.radius * (0.35 + progress * 0.9);
    ctx.save();
  ...
  function drawGame() {
    ...
    if (game.dangerPulse > 0) {
      ctx.save();
      const alpha = 0.16 + Math.abs(Math.sin(now / 85)) * 0.18;
      ctx.globalAlpha = alpha;
  ...
  function zombieTypeForWave() {
    ...
    const pressure = game.difficulty === "nightmare" ? 1.25 : game.difficulty === "normal" ? 0.82 : 1;
    const nightmare = game.difficulty === "nightmare";
    const choices = [
      [10, Math.min(0.18, 0.07 + (game.wave - 9) * 0.035) * pressure, "gargantuar"],
      [8, Math.min(0.18, (0.035 + game.wave * 0.012) * pressure), "dragon"],
      [7, Math.min(0.22, (0.04 + game.wave * 0.014) * pressure), "witch"],
      [6, Math.min(0.24, (0.05 + game.wave * 0.016) * pressure), "shield"],
      [3, Math.min(0.24, (0.04 + game.wave * 0.012) * pressure), "conehead"],
      [2, Math.min(0.28, (0.07 + game.wave * 0.016) * pressure), "imp"],
      [4, Math.min(0.18, (0.025 + game.wave * 0.011) * pressure), "scout"],
      [6, Math.min(0.16, (0.02 + game.wave * 9e-3) * pressure), "storm"],
      [4, Math.min(0.2, (0.05 + game.wave * 0.01) * pressure), "newspaper"],
      [7, Math.min(0.16, (0.025 + game.wave * 0.01) * pressure), "dancer"],
      [5, Math.min(0.3, (0.045 + game.wave * 0.018) * pressure), "football"],
      [4, Math.min(0.22, (0.04 + game.wave * 0.014) * pressure), "polevault"],
      [4, Math.min(0.25, (0.035 + game.wave * 0.014) * pressure), "miner"],
      [3, Math.min(0.22, (0.045 + game.wave * 0.01) * pressure), "flag"],
      [3, Math.min(0.38, (0.11 + game.wave * 0.014) * pressure), "bucket"],
      [2, Math.min(0.52, (0.25 + game.wave * 0.018) * pressure), "runner"],
      [2, Math.min(0.82, (0.42 + game.wave * 0.022) * pressure), "roadblock"]
    ];
  ...
  function spawnZombie() {
    ...
    const hpGrowth = type === "gargantuar" ? 2.2 : type === "dragon" ? 1.55 : type === "witch" ? 1.2 : type === "shield" ? 1.25 : type === "football" ? 1.35 : type === "bucket" ? 1.2 : type === "miner" ? 1 : type === "roadblock" ? 0.85 : 0.7;
    ...
    game.zombies.push({
      ...
      slowTimer: 0,
      burrowTimer: profile.burrow ? 1e3 : 0,
    ...
    if (game.waveSpawned === game.waveTarget) {
      playGameSound("wave");
      addGameParticle(360, 60, "#ffe27c", 18, 0.18);
    }
    updateGameHud(true);
  }
  function produceSun(plant) {
    const position = cellPosition(plant.row, plant.col);
    game.suns.push({ x: position.x + (Math.random() - 0.5) * 24, y: position.y - 28, age: 0, targetY: position.y - 5 });
    addGameParticle(position.x, position.y - 22, "#ffe17b", 8, 0.12);
  }
  function collectSunAt(index, x, y) {
    ...
    game.sun += 25;
    updateGameHud(true);
    addGameParticle(x, y, "#ffe17b", 12, 0.16);
    playGameSound("collect");
    return true;
  }
  ...
  function explodeCherryBomb(plant) {
    ...
    addGameParticle(position.x, position.y - 5, "#ff8d73", 34, 0.3);
    playGameSound("explode");
    updateGameHud();
  }
  function defeatZombie(zombie, source = "combat") {
    ...
    const comboText = game.combo > 1 ? state.locale === "zh" ? `+${points} \xB7 ${game.combo} \u8FDE\u51FB` : `+${points} \xB7 x${game.combo}` : `+${points}`;
  ...
  function triggerMower(mower) {
    ...
    addGameParticle(mower.x + 12, cellPosition(mower.row, 0).y + 18, "#ffb16c", 16, 0.22);
    playGameSound("mower");
    updateGameHud(false);
    return true;
  }
  ...
  function damagePlant(plant, amount, color = "#c78363") {
    ...
    const effectiveAmount = target.type === "pumpkin" ? amount * 0.5 : amount;
    target.hp -= effectiveAmount;
    const position = cellPosition(target.row, target.col);
    addGameParticle(position.x, position.y - 8, color, 4, 0.1);
    if (target.hp > 0) return false;
    removeGamePlant(target);
    addGameParticle(position.x, position.y, color, 12, 0.16);
    return true;
  }
  function curseNearestPlant(zombie, duration = 3e3) {
    const plants = rowEntities("plants", zombie.row);
    let target = null;
    let targetX = -Infinity;
    for (let index = 0; index < plants.length; index += 1) {
      const plant = plants[index];
      const position2 = cachedCellPosition(plant);
      if (position2.x < zombie.x && position2.x > targetX) {
        target = plant;
        targetX = position2.x;
      }
    }
    ...
    addGameParticle(position.x, position.y - 24, "#c99be8", 14, 0.14);
    return true;
  }
  function gameLoop(now = 0) {
    ...
    game.suns.forEach((sun) => {
      sun.age += dt;
      if (sun.y < sun.targetY) sun.y = Math.min(sun.targetY, sun.y + dt * 0.05);
    });
    ...
    plantsThisFrame.forEach((plant) => {
      ...
      if (plant.type === "sunflower") {
        plant.sunTimer += dt;
        if (plant.sunTimer > 4800 && game.suns.length < 12) {
          plant.sunTimer = 0;
          produceSun(plant);
        }
        return;
      ...
      if (plant.type === "jalapeno") {
        ...
        if (plant.bombTimer > 850) {
          ...
          removeGamePlant(plant);
          addGameParticle(position2.x + 180, position2.y, "#ff784e", 40, 0.35);
          playGameSound("explode");
          updateGameHud();
        }
        return;
      ...
      if (plant.type === "potatomine") {
        ...
        if (!plant.armed && plant.bombTimer >= 1800) {
          plant.armed = true;
          addGameParticle(cellPosition(plant.row, plant.col).x, cellPosition(plant.row, plant.col).y - 18, "#e7c875", 8, 0.08);
        }
        ...
        if (target) {
          ...
          removeGamePlant(plant);
          addGameParticle(cellPosition(plant.row, plant.col).x, cellPosition(plant.row, plant.col).y, "#e7c875", 26, 0.25);
          playGameSound("explode");
          updateGameHud();
        }
        return;
      ...
      if (plant.type === "spikeweed") {
        plant.shotTimer += dt;
        const position2 = cellPosition(plant.row, plant.col);
        const target = game.zombies.find((zombie) => zombie.row === plant.row && Math.abs(zombie.x - position2.x) < 44);
        if (target) {
          target.hp -= dt / 720;
          if (target.hp <= 0) {
            defeatZombie(target);
      ...
      if (plant.type === "gloomshroom") {
        plant.shotTimer += dt;
        const position2 = cellPosition(plant.row, plant.col);
        if (plant.shotTimer > 1050) {
          plant.shotTimer = 0;
          const targets = game.zombies.filter((zombie) => Math.abs(zombie.row - plant.row) <= 1 && Math.hypot(zombie.x - position2.x, (zombie.row - plant.row) * gameLayout.cellH) < 140);
          targets.forEach((target) => {
            target.hp -= 1;
            addGameParticle(target.x, target.y - 12, "#c79be8", 4, 0.1);
            if (target.hp <= 0) defeatZombie(target);
          });
          if (targets.length) playGameSound("shoot");
        }
        return;
      }
      ...
      if (plant.type === "squash") {
        plant.bombTimer += dt;
        const position2 = cellPosition(plant.row, plant.col);
        const target = game.zombies.find((zombie) => zombie.row === plant.row && zombie.x > position2.x - 82 && zombie.x < position2.x + 130);
        if (plant.bombTimer > 450 && target) {
          defeatZombie(target);
          removeGamePlant(plant);
          addGameParticle(target.x, target.y, "#f0b653", 28, 0.28);
          playGameSound("explode");
          updateGameHud();
        }
        return;
      ...
      plant.shotTimer += dt;
      const position = cellPosition(plant.row, plant.col);
      const rowThreat = profile.rows ? anyRowEntity("zombies", plant.row, (zombie) => zombie.x > position.x) || plant.row > 0 && anyRowEntity("zombies", plant.row - 1, (zombie) => zombie.x > position.x) || plant.row + 1 < gameLayout.rows && anyRowEntity("zombies", plant.row + 1, (zombie) => zombie.x > position.x) : anyRowEntity("zombies", plant.row, (zombie) => zombie.x > position.x);
      ...
      if (plant.shotTimer > fireInterval && rowThreat) {
        ...
        if (profile.utility) {
          ...
          if (armored) {
            ...
            addGameParticle(armored.x, armored.y - 20, "#dcb7ff", 15, 0.15);
            playGameSound("hit");
          }
    ...
    game.zombies.forEach((zombie) => {
      if (zombie.type === "flag") game.flagRows[zombie.row] = 1;
    });
    ...
    zombiesThisFrame.forEach((zombie) => {
      ...
      if (zombie.burnTimer > 0 && zombie.burnTickTimer <= 0) {
        ...
        addGameParticle(zombie.x, zombie.y - 18, "#ff815f", 4, 0.08);
        if (zombie.hp <= 0) {
          defeatZombie(zombie);
          updateGameHud();
          return;
      ...
      if (["runner", "imp", "scout"].includes(zombie.type) && zombie.dashTimer <= 0 && zombie.x < 650) {
        zombie.dashTimer = zombie.type === "imp" ? 2100 : 3e3;
        zombie.x += zombie.type === "imp" ? 38 : 28;
        zombie.flashTimer = 140;
        addGameParticle(zombie.x, zombie.y - 22, zombie.type === "scout" ? "#f5cf63" : "#e57b70", 8, 0.14);
      }
      if (zombie.type === "football" && zombie.chargeTimer <= 0) {
        zombie.chargeTimer = 3e3;
        zombie.flashTimer = 160;
        addGameParticle(zombie.x, zombie.y - 20, "#d56c58", 8, 0.13);
      }
      if (zombie.type === "witch" && zombie.curseTimer <= 0) {
        zombie.curseTimer = 4300;
        curseNearestPlant(zombie, zombie.elite ? 4200 : 3e3);
      }
      if (zombie.type === "dragon" && zombie.breathTimer <= 0) {
        ...
        addGameParticle(zombie.x - 34, zombie.y - 12, "#ff9b5f", 18, 0.22);
      }
      if (zombie.type === "shield" && zombie.armorTimer <= 0) {
        ...
        addGameParticle(zombie.x, zombie.y - 24, "#9bdcf5", 12, 0.14);
      }
      if (zombie.type === "storm" && zombie.stormTimer <= 0) {
        zombie.stormTimer = 4e3;
        game.plants.filter((plant) => plant.row === zombie.row).forEach((plant) => {
          plant.disabledTimer = Math.max(plant.disabledTimer || 0, 1200);
        });
        addGameParticle(zombie.x - 24, zombie.y - 28, "#a9c8e8", 20, 0.2);
      }
      ...
      if (zombie.type === "dancer") {
        ...
        if (zombie.summonTimer > 4200) {
          zombie.summonTimer = 0;
          const allyRow = (zombie.row + 1) % gameLayout.rows;
          const ally = zombieProfiles.backup;
          game.zombies.push({ x: zombie.x + 34, y: cellPosition(allyRow, 0).y, row: allyRow, hp: ally.hp, maxHp: ally.hp, armor: 0, type: "backup", speed: ally.speed, attackInterval: ally.attackInterval, slowTimer: 0, burrowTimer: 0, seed: Math.random() * 1e3, garlicTimer: 0, vaultTimer: 0, summonTimer: 0, flashTimer: 0, attackTimer: 0, attacking: false, staggerTimer: 0 });
          addGameParticle(zombie.x, zombie.y - 28, "#ef7892", 16, 0.18);
          playGameSound("wave");
        }
      }
      ...
      if (zombie.type === "imp" && blocker && zombie.leapTimer <= 0) {
        zombie.x = cellPosition(blocker.row, blocker.col).x - 44;
        zombie.leapTimer = 1800;
        addGameParticle(zombie.x, zombie.y - 25, "#e57b70", 12, 0.16);
        return;
      }
      if (blocker?.type === "spikeweed") {
        zombie.hp -= dt / 720;
        if (zombie.hp <= 0) {
          defeatZombie(zombie);
          updateGameHud();
      ...
      if (zombie.type === "polevault" && blocker && !zombie.vaultTimer) {
        zombie.x = cellPosition(blocker.row, blocker.col).x - 44;
        zombie.vaultTimer = 1;
        addGameParticle(zombie.x, zombie.y - 25, "#d5a15e", 12, 0.16);
        return;
      }
      if (blocker?.type === "garlic" && zombie.garlicTimer <= 0) {
        zombie.row = (zombie.row + 1) % gameLayout.rows;
        zombie.x += 22;
        zombie.garlicTimer = 3200;
        addGameParticle(zombie.x, zombie.y, "#f3e1b4", 14, 0.16);
        playGameSound("hit");
        return;
      }
      if (zombie.type === "gargantuar" && blocker && zombie.smashTimer <= 0) {
        zombie.smashTimer = zombie.elite ? 2200 : 3e3;
        damagePlant(blocker, zombie.elite ? 10 : 7, "#d99a5e");
        addGameParticle(zombie.x, zombie.y - 20, "#d99a5e", 18, 0.22);
        playGameSound("explode");
        return;
      }
      if (blocker) {
        ...
        const giantSlow = zombie.type === "gargantuar" ? 0.72 : 1;
        zombie.x -= zombie.speed * bannerBoost * enragedBoost * dashBoost * chargeBoost * giantSlow * dt * (zombie.slowTimer > 0 ? 0.48 : 1);
      }
    });
    updateMowers(dt);
    game.shots.forEach((shot) => {
      ...
      addGameParticle(shot.x, shot.y, shot.color || "#b7f3a0", 5, 0.1);
      playGameSound("hit");
      if (hit.hp <= 0) {
        defeatZombie(hit);
        updateGameHud();
        addGameParticle(hit.x, hit.y, "#f6d681", 18, 0.2);
      }
    });
    ...
    if (game.zombies.some((zombie) => zombie.x < 61)) {
      addGameParticle(55, 180, "#ff9f83", 24, 0.22);
  ...
  function canvasPoint(event) {
    const canvas = gameRender.canvas || $("#gameCanvas"), rect = canvas.getBoundingClientRect();
    return { x: (event.clientX - rect.left) * GAME_LOGICAL_WIDTH / rect.width, y: (event.clientY - rect.top) * GAME_LOGICAL_HEIGHT / rect.height };
  }
  ...
  function collectSun(event) {
    ...
    const hit = game.suns.findIndex((sun2) => Math.hypot(sun2.x - x, sun2.y - y) < 32);
    if (hit < 0) return false;
    const sun = game.suns[hit];
    collectSunAt(hit, sun.x, sun.y);
    drawGame();
    return true;
  }
  function gameCellAt(x, y) {
    if (y < gameLayout.top || x < gameLayout.left) return null;
    const col = Math.floor((x - gameLayout.left) / gameLayout.cellW), row = Math.floor((y - gameLayout.top) / gameLayout.cellH);
    return col >= 0 && col < gameLayout.cols && row >= 0 && row < gameLayout.rows ? { row, col } : null;
  }
  function plantAt(event) {
    ...
    if (game.shovel) {
      ...
      const position2 = cellPosition(existing.row, existing.col);
      removeGamePlant(existing);
      game.shovel = false;
      updateShovelButton();
      addGameParticle(position2.x, position2.y, "#e7d7a0", 12, 0.14);
      playGameSound("hit");
      drawGame();
      return true;
    }
    ...
    if (game.sun < cost) {
      setGameStatus("game.noSun");
      return false;
    }
    if ((game.seedCooldowns[type] || 0) > 0) {
      setGameStatus("game.cooldown");
      return false;
    }
    game.sun -= cost;
    game.seedCooldowns[type] = plantCooldown[type] || 0;
    const planted = { type, hp: plantHealth[type], row: cell.row, col: cell.col, seed: Math.random() * 1e3, age: 0, sunTimer: 0, shotTimer: 0, bombTimer: 0, disabledTimer: 0, armed: type !== "potatomine" };
    ...
    updateGameHud();
    clearPlantSelection();
    addGameParticle(position.x, position.y, plantColor[type] || "#fff", 12, 0.12);
    playGameSound("plant");
    drawGame();
    return true;
  }
  ...
  function openGameWindow() {
    window.open(location.origin + location.pathname + "?arcade=1", "minicc-arcade", "popup,width=980,height=760");
  }
  function toggleGameFullscreen() {
    const card = $("#gameModal .game-card");
  ...
  var codexState = { tab: "plants", plant: "peashooter", zombie: "walker" };
  var codexPlantNames = { peashooter: "\u8C4C\u8C46\u5C04\u624B", sunflower: "\u5411\u65E5\u8475", wallnut: "\u575A\u679C\u5899", repeater: "\u53CC\u53D1\u5C04\u624B", cherrybomb: "\u6A31\u6843\u70B8\u5F39", icepeashooter: "\u5BD2\u51B0\u5C04\u624B", firepeashooter: "\u706B\u7130\u5C04\u624B", twinpea: "\u53CC\u53D1\u5F3A\u5316", kernelpult: "\u7389\u7C73\u6295\u624B", pumpkin: "\u5357\u74DC\u5934", spikeweed: "\u5730\u523A", gloomshroom: "\u5FE7\u90C1\u83C7", potatomine: "\u571F\u8C46\u96F7", threepeater: "\u4E09\u7EBF\u5C04\u624B", jalapeno: "\u706B\u7206\u8FA3\u6912", magnetshroom: "\u78C1\u529B\u83C7", garlic: "\u5927\u849C", squash: "\u7A9D\u74DC", gatlingpea: "\u673A\u67AA\u5C04\u624B" };
  var codexZombieNames = { walker: "\u666E\u901A\u50F5\u5C38", backup: "\u4F34\u821E\u50F5\u5C38", roadblock: "\u8DEF\u969C\u50F5\u5C38", conehead: "\u8DEF\u9525\u50F5\u5C38", imp: "\u5C0F\u9B3C\u50F5\u5C38", scout: "\u4FA6\u5BDF\u50F5\u5C38", storm: "\u98CE\u66B4\u50F5\u5C38", runner: "\u5954\u8DD1\u50F5\u5C38", polevault: "\u6491\u6746\u50F5\u5C38", bucket: "\u94C1\u6876\u50F5\u5C38", football: "\u6A44\u6984\u7403\u50F5\u5C38", miner: "\u77FF\u5DE5\u50F5\u5C38", flag: "\u65D7\u5E1C\u50F5\u5C38", dancer: "\u821E\u738B\u50F5\u5C38", newspaper: "\u62A5\u7EB8\u50F5\u5C38", gargantuar: "\u5DE8\u4EBA\u50F5\u5C38", witch: "\u5973\u5DEB\u50F5\u5C38", dragon: "\u9F99\u50F5\u5C38", shield: "\u62A4\u76FE\u50F5\u5C38" };
  var codexPlantIcons = { peashooter: "\u{1F331}", sunflower: "\u{1F33B}", wallnut: "\u{1F95C}", repeater: "\u{1F33F}", cherrybomb: "\u{1F352}", icepeashooter: "\u2744\uFE0F", firepeashooter: "\u{1F525}", twinpea: "\u{1F331}", kernelpult: "\u{1F33D}", pumpkin: "\u{1F383}", spikeweed: "\u{1F335}", gloomshroom: "\u{1F344}", potatomine: "\u{1F954}", threepeater: "\u{1F33E}", jalapeno: "\u{1F336}\uFE0F", magnetshroom: "\u{1F9F2}", garlic: "\u{1F9C4}", squash: "\u{1F383}", gatlingpea: "\u{1F52B}" };
  var codexPlantSpecials = { peashooter: "\u53D1\u5C04\u666E\u901A\u8C4C\u8C46\uFF0C\u7A33\u5B9A\u8F93\u51FA\u3002", sunflower: "\u6BCF\u9694\u4E00\u6BB5\u65F6\u95F4\u751F\u4EA7 25 \u9633\u5149\u3002", wallnut: "\u9AD8\u751F\u547D\u503C\u963B\u6321\uFF0C\u62D6\u5EF6\u50F5\u5C38\u3002", repeater: "\u6BCF\u8F6E\u53D1\u5C04 2 \u53D1\u8C4C\u8C46\uFF0C\u5E76\u53EF\u7A7F\u900F 1 \u4E2A\u76EE\u6807\u3002", cherrybomb: "\u77ED\u5EF6\u8FDF\u540E\u5728\u540C\u4E00\u884C 145 \u8303\u56F4\u5185\u76F4\u63A5\u6D88\u706D\u50F5\u5C38\u3002", icepeashooter: "\u547D\u4E2D\u540E\u51CF\u901F 3200ms\uFF0C\u5E76\u53EF\u7A7F\u900F 1 \u4E2A\u76EE\u6807\u3002", firepeashooter: "\u6BCF\u53D1 2 \u70B9\u4F24\u5BB3\u5E76\u65BD\u52A0 2600ms \u707C\u70E7\uFF0C\u707C\u70E7\u4F24\u5BB3 3\u3002", twinpea: "\u6BCF\u8F6E\u53D1\u5C04 2 \u53D1\u5F3A\u5316\u8C4C\u8C46\uFF0C\u6BCF\u53D1 2 \u70B9\u4F24\u5BB3\u3002", kernelpult: "28% \u6982\u7387\u7528\u9EC4\u6CB9\u5B9A\u8EAB\uFF0C\u5E76\u53EF\u7A7F\u900F 1 \u4E2A\u76EE\u6807\u3002", pumpkin: "\u4E3A\u540C\u683C\u690D\u7269\u63D0\u4F9B 32 \u70B9\u62A4\u7F69\u751F\u547D\u3002", spikeweed: "\u653B\u51FB\u6240\u5728\u683C\u9644\u8FD1 44 \u8303\u56F4\u5185\u7684\u50F5\u5C38\u3002", gloomshroom: "\u8FD1\u8EAB\u8303\u56F4\u653B\u51FB\u5E76\u65BD\u52A0 900ms \u51CF\u901F\u3002", potatomine: "1800ms \u540E\u5E03\u96F7\uFF0C\u5728\u540C\u4E00\u884C 90 \u8303\u56F4\u5185\u7206\u70B8\u3002", threepeater: "\u540C\u65F6\u653B\u51FB\u5F53\u524D\u884C\u3001\u4E0A\u884C\u548C\u4E0B\u884C\u3002", jalapeno: "\u77ED\u5EF6\u8FDF\u540E\u6D88\u706D\u6240\u5728\u884C\u7684\u5168\u90E8\u50F5\u5C38\u3002", magnetshroom: "\u5468\u671F\u6027\u5438\u8D70\u50F5\u5C38\u62A4\u7532\u6216\u88C5\u5907\uFF0C\u4E0D\u76F4\u63A5\u9020\u6210\u4F24\u5BB3\u3002", garlic: "\u88AB\u54AC\u540E\u5C06\u50F5\u5C38\u6539\u9053\u5230\u4E0B\u4E00\u884C\u3002", squash: "\u63A5\u8FD1\u65F6\u91CD\u51FB\u5E76\u76F4\u63A5\u6D88\u706D\u76EE\u6807\u3002", gatlingpea: "\u6BCF\u8F6E\u8FDE\u7EED\u53D1\u5C04 4 \u53D1\u8C4C\u8C46\uFF0C\u6BCF\u53D1 1 \u70B9\u4F24\u5BB3\u3002" };
  var codexZombieSkills = { walker: "\u65E0\u989D\u5916\u6280\u80FD\uFF0C\u63A5\u89E6\u690D\u7269\u540E\u5543\u98DF\u3002", backup: "\u4F34\u968F\u821E\u738B\u53EC\u5524\uFF0C\u6CBF\u884C\u5543\u98DF\u3002", roadblock: "\u8DEF\u969C\u63D0\u4F9B\u989D\u5916\u9632\u62A4\u3002", conehead: "\u8DEF\u9525\u63D0\u4F9B\u989D\u5916\u62A4\u7532\u3002", imp: "\u5FEB\u901F\u79FB\u52A8\u5E76\u8DF3\u8DC3\u690D\u7269\u3002", scout: "\u95F4\u6B47\u51B2\u523A\u5E76\u6807\u8BB0\u3001\u8BC5\u5492\u9644\u8FD1\u690D\u7269\u3002", storm: "\u5468\u671F\u6027\u4F7F\u540C\u4E00\u884C\u690D\u7269\u77ED\u6682\u5931\u6548\u3002", runner: "\u6CBF\u884C\u5FEB\u901F\u79FB\u52A8\u5E76\u95F4\u6B47\u51B2\u523A\u3002", polevault: "\u9047\u5230\u7B2C\u4E00\u682A\u690D\u7269\u65F6\u6491\u6746\u8DF3\u8FC7\u3002", bucket: "\u94C1\u6876\u63D0\u4F9B\u9AD8\u989D\u62A4\u7532\u3002", football: "\u9AD8\u62A4\u7532\u5E76\u53EF\u51B2\u950B\u653B\u51FB\u3002", miner: "\u5730\u4E0B\u6F5C\u884C\uFF0C\u63A5\u8FD1\u9632\u7EBF\u540E\u51FA\u571F\u3002", flag: "\u4E3A\u540C\u4E00\u884C\u76DF\u53CB\u63D0\u4F9B\u79FB\u52A8\u901F\u5EA6\u52A0\u6210\u3002", dancer: "\u5468\u671F\u6027\u53EC\u5524\u4F34\u821E\u50F5\u5C38\u3002", newspaper: "\u62A5\u7EB8\u88AB\u7834\u574F\u540E\u8FDB\u5165\u72C2\u66B4\u72B6\u6001\u3002", gargantuar: "\u7F13\u6162\u63A8\u8FDB\uFF0C\u63A5\u89E6\u690D\u7269\u65F6\u91CD\u51FB\u5E76\u9020\u6210\u9AD8\u989D\u4F24\u5BB3\u3002", witch: "\u6807\u8BB0\u5E76\u8BC5\u5492\u9644\u8FD1\u690D\u7269\u3002", dragon: "\u55B7\u5410\u706B\u7130\uFF0C\u5BF9\u690D\u7269\u65BD\u52A0\u707C\u70E7\u3002", shield: "\u5468\u671F\u6027\u6062\u590D\u62A4\u76FE\u3002" };
  ...
  function codexRows(rows) {
    return rows.map(([label, value]) => `<div class="codex-row"><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(String(value))}</dd></div>`).join("");
  }
